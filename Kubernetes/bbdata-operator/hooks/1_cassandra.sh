...
if [[ $1 == "--config" ]] ; then
  ...
  # Check if cassandra replica env variable is set and set it to default value otherwise
  if [[ -z "${CASSANDRA_REPLICA_SET}" ]]; then
    CASSANDRA_REPLICA_SET=$DEFAULT_REPLICA_SET
  fi

  # Check if cassandra StrorageClass name is set and set it to 1st storageclass name otherwise
  if [[ -z "${CASSANDRA_STORAGECLASS_NAME}" ]]; then
    CASSANDRA_STORAGECLASS_NAME=$(kubectl get sc -o "jsonpath={.items[0].metadata.name}")
  fi
